...
for(i in 1:length(files)){
	...
	q <- ggplot(df.m) + geom_density(aes(x=value,colour=variable)) + labs(x = "Age", y = "Density") + scale_colour_discrete(name = "Age", breaks=c("fam_age","len_age"), labels=c("Family Age", "Length Age")) + ggtitle(titles[i])
...
q <- ggplot(df.m1) + geom_density(aes(x=value,colour=variable)) + labs(x = "Age", y = "Density") + scale_colour_discrete(name = "Age", breaks = c("fam_age", "corr_age"), labels = c("Family Age", "Corrected Family Age")) + ggtitle("Pan-taxa Compara Tree Correction")
svg(filename = paste(substr(file,1,nchar(file)-4),'_fam_correction','.svg',sep = ''))
print(q)

df.m2 <- melt(df, measure.vars = c('len_age','corr_len_age'))
q <- ggplot(df.m2) + geom_density(aes(x=value,colour=variable)) + labs(x = "Age", y = "Density") + scale_colour_discrete(name = "Age", breaks = c("len_age", "corr_len_age"), labels = c("Length Age", "Corrected Length Age")) + ggtitle("Pan-taxa Compara Tree Correction")
...
q <- ggplot(df) + geom_density(aes(x = fam_age, color = Cancer_Molecular_Genetics)) + labs(x = "Age", y = "Density") + scale_colour_discrete(name = "Cancer Gene Type", breaks = c("Dom", "Rec"), labels = c("Dominant", "Recessive")) + ggtitle("Family Age in COSMIC Cancer Genes")
p <- ggplot(df) + geom_boxplot(aes(x = Cancer_Molecular_Genetics, y = fam_age, color = Cancer_Molecular_Genetics)) + labs(x = "Cancer Gene Type", y = "Family Age") + scale_colour_discrete(name = "Cancer Gene Type", breaks = c("Dom", "Rec"), labels = c("Dominant", "Recessive")) + ggtitle("Family Age in COSMIC Cancer Genes")
